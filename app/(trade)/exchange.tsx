...
import { ActionSheetItem, Button, Input, Modal, useActionSheet } from "@/components/ui"
...
export default function ExchangePage() {
    ...
    const handleSaveQRCode = useCallback(async () => {
        try {
            ...
            if (status !== 'granted') return setIsGrantAccess(true)
            const base64 = await getQRBase64()
            const fileUri = FileSystem.cacheDirectory + 'qr.png'
            await FileSystem.writeAsStringAsync(fileUri, base64, { encoding: FileSystem.EncodingType.Base64, })
            await MediaLibrary.saveToLibraryAsync(fileUri)
            setIsSaving(false)
            setSaveResult({title: 'exchange.saveQrcodeToast.title', content: 'exchange.saveQrcodeToast.successContent'})
        } catch (error) {
            setIsSaving(false)
            setSaveResult({title: 'exchange.saveQrcodeToast.title', content: 'exchange.saveQrcodeToast.errorContent'})
        } 
    }, [t])
